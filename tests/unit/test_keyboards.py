...
def test_status_button_text_is_status_request() -> None:
    ...
    assert is_setup_request(SETUP_BUTTON_TEXT)
    assert is_settings_request(SETTINGS_BUTTON_TEXT)
    assert is_my_programs_request(MY_PROGRAMS_BUTTON_TEXT)
    assert is_all_programs_request(ALL_PROGRAMS_BUTTON_TEXT)
    assert is_general_only_request(GENERAL_ONLY_BUTTON_TEXT)
    assert is_all_categories_request(ALL_CATEGORIES_BUTTON_TEXT)


def test_max_keyboard_groups_status_profile_and_scope_buttons() -> None:
...
def test_telegram_keyboard_groups_status_profile_and_scope_buttons() -> None:
