...
По умолчанию ответ компактный и не объясняет алгоритм расчета. Подробная выдача включается
командой `/debug`; в ней дополнительно показываются источник, строки с баллами, расчет,
фильтрация по приоритетам и прогноз.
...
| `/status` | статус вне приоритетов | разрешённый пользователь |
| `/relative`, `/relative_status` | относительный статус с учетом более высоких приоритетов | разрешённый пользователь |
...
| `/debug`, `/debug on`, `/debug off` | переключение подробной выдачи статуса | владелец |
...
Относительный статус строит временные списки по текущему выбранному режиму категорий: абитуриент
с приоритетом `2..5` исключается из нижнего приоритета только если тот же код заявки уверенно
проходит в списке с более высоким приоритетом. При равных баллах на границе заявка не исключается,
пока весь интервал равных баллов не помещается в доступные места.
...
```text
/debug
Подробный режим включен. Следующий статус покажет источник, расчет, фильтрацию и прогноз.

/debug off
Подробный режим выключен. Следующий статус будет компактным.
```

В компактном режиме статус показывает только пользовательский результат: места, заявления,
приоритет, позицию, текущий проходной, статус и достоверность. В подробном режиме статус
дополнительно печатает источник данных, количество строк с баллами, нижнюю границу,
черновой прогноз и пояснение относительной фильтрации.
...
3. Доступ только владельцу или также членам семьи? - зависит от настройки конкретного deployment.
4. Какая разбивка баллов по предметам? - не хранится в публичной документации.
5. Есть ли индивидуальные достижения? - задается в профиле чата.
6. Известен ли анонимный номер заявления? - задается в профиле чата.
7. Нужны ли напоминания о дедлайнах независимо от списков? - отдельная продуктовая настройка.
8. Хранить ли raw responses дольше 7 дней? - отдельная эксплуатационная настройка.
10. Нужен ли Excel-экспорт по завершении кампании? - отдельная продуктовая настройка.
