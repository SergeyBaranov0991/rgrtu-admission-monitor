...
1. публичный JSON/XHR endpoint официального интерфейса;
2. серверный HTML;
3. Playwright, если данные формируются JavaScript;
4. последний успешный снимок при временной недоступности.
...
### Вариант B — общий VPS и SNI routing

Nginx Stream, HAProxy или XRay fallback. Требуется отдельный домен, резервный доступ, тест VPN и план отката.
...
| Только JS | средняя | средняя | Playwright fallback |
| Нет JSON endpoint | средняя | средняя | HTML/browser adapter |
...
```bash
docker compose exec bot python -m app.cli check --score 195
```
...
Порядок миграции должен исключать запуск нового несовместимого кода до обновления схемы.
